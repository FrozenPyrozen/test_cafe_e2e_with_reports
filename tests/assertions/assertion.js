...
fixture("Assertion API").page("https://demoqa.com/elements");
...
test.skip("Deep Equal ", async (t) => {
  await t
    ...
    .eql(6, "Accordion Items Count", { timeout: 2000 });
});

test.skip("NOT Deep Equal ", async (t) => {
  await t
    .expect(Selector(".header-wrapper").count)
    .notEql(5, "Accordion Items Count", { timeout: 2000 });
});
...
test.skip("Ok ", async (t) => {
  ...
  await t
    ...
    .ok('This test will validate if the "submit" button exist');
});
...
test.skip("Not Ok ", async (t) => {
  ...
  await t
    .expect(Selector("#submit").exists)
    .notOk('This test will validate if the "submit" button exist');
});

test("Contains ", async (t) => {
  const getLocation = ClientFunction(() => document.location.href.toString());

  //await t.click('#item-0');d
